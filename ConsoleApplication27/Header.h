...
public:
	...
	std::vector <Student> resultStudent;
	int reserve = 0;
	int day;
...
void student_allocation(vector<Elective> electives, vector <Student> students, int min_capacity =0, int quota=0)
{
	...
	{ //впервые пользуюсь ссылками DANGEROUS
		for (int prior = 0;( prior < students_copy[i].priorites.size()) && !(students_copy[i].firstAvailable && students_copy[i].secondAvailable); prior++) // заполн€ем студентов на элективы, пока есть возможность
		{
	...
	for (int i = 0; i < students_copy.size(); i++)
	{
		...
		for (int it_el = 0; it_el<electives.size() && !(students_copy[i].firstAvailable && students_copy[i].secondAvailable); it_el++) // заполн€ем оставшихс€ студентов на элективы
		{
			Student& temp_stud = students[students_copy[i].id];
...
// ¬ы бы знали как € себ€ сдерживаю, чтобы не начать писать гениальный код. Ќикто бы не пон€л его, даже €. ј так хочетс€ подсократить строчки.
// “ак хочетс€ for each, но цена ошибки высока.
...
void Floyd(vector<Elective> electives, vector <Student> students) {
	vector<vector<vector<pair<int,int>>>> tapki_bratki(electives.size(),vector<vector<pair<int,int>>>(electives.size(), vector<pair<int, int>>(electives.size(), { 0,0 }))); // “€жЄла€ игра
